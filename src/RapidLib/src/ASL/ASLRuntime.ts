/**
 * Async Script Loader (Web version)
 * 
 * @randomuserhi 2025
 */
...
export let ASL_BASE_URL: string | URL | undefined = undefined;
...
export class ASLEnvironment {
    ...
    private import(promise: CancellablePromise<ASLModuleObject>, module: ASLModule, path: string, options?: ASLImportOptions): Promise<ASLModuleObject> {
        ...
        // Resolve paths
        path = new URL(path, path.startsWith(".") ? module.path : ASL_BASE_URL).toString();
        ...
        throw new ASLImportError(`Invalid import type '${parsedOptions.type}'.`);
    }
